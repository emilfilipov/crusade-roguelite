...
| Player Factions | Match setup exposes `Christian` and `Muslim` as playable factions; selected faction determines commander/recruit pool and opposite-faction enemy pool | Additional faction-exclusive mechanics, progression paths, and content depth | Dual-faction parity remains stable under full-run tests |
