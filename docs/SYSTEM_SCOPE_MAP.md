...
| Recruit Roster | 3 recruit variants: `Christian Peasant Infantry` (melee), `Christian Peasant Archer` (hybrid), and `Christian Peasant Priest` (non-damaging support caster with periodic attack-speed blessing) | Spearman, cavalry, elite supports, faction variants | Rescue/promotion loop stable and roster growth bug-free |
...
| Rescue System | 1 rescue interaction: any friendly unit in radius for fixed time; data-driven `recruit_pool` currently restricted to tier-0 units only | Escort rescues, contested rescues, rescue events, higher-tier or faction-specific rescue pools | Base rescue flow has no stuck/interruption bugs |
...
| Unit Upgrade Screen | In-run modal (`U`) with roster source selection, tier-path scaffold, and bulk promotion actions (`+1/+5/MAX`) constrained by level-budget affordability | Full node-graph tree with richer branching, rarity, and previews | Current promotion economy + UI interactions remain deterministic and test-covered |
